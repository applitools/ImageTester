...
public class EyesFactory {
	...
	public synchronized Eyes build() throws RuntimeException {
		...
		// should be set first
		eyes.setConfiguration(new Configuration().setDeviceInfo(this.deviceName));
	...
	public EyesFactory imageCut(String[] cutArgs) {
		...
		if (cutArgs.length > 4)
			throw new IllegalArgumentException(
					"Image cut must contain at most four parameters (header,footer,left,right)");
		this.cutValues = Arrays.stream(cutArgs).mapToInt((s) -> StringUtils.isNumeric(s) ? Integer.parseInt(s) : 0)
				.toArray();
		return this;
	}
	...
	public EyesFactory accSettings(String[] settings) {
		...
		if (settings.length > 0) {
			level = StringUtils.isNotBlank(settings[0]) ? Utils.parseEnum(AccessibilityLevel.class, settings[0], "_")
					: level;
