...
export const OPTION_SPECS: OptionSpec[] = [
  ...
  { flag: "nf",  label: "Normalize Latin fonts",    type: "checkbox", tab: "pdf", help: "Redraw Latin-script PDF text in Helvetica, keeping every glyph's original size and position — ignores font-embedding differences, not layout changes. Japanese text is left untouched (use Normalize Japanese fonts). Invalidates existing baselines.", default: false },
  { flag: "nfj", label: "Normalize Japanese fonts", type: "checkbox", tab: "pdf", help: "Redraw Japanese text (Hiragana, Katakana, Kanji) in a bundled Noto Sans JP, keeping every glyph's original size and position — ignores Japanese font-embedding differences, not layout changes. Combine with Normalize Latin fonts for full normalization. Invalidates existing baselines.", default: false },
